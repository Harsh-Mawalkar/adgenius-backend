...
const matchSchema = new mongoose.Schema({
    adId: { type: mongoose.Schema.Types.ObjectId, ref: 'Campaign' },
...
app.post("/api/websites", async (req, res) => {
    ...
    try {
      ...
      res.status(201).json({ message: "Website stored in MongoDB Atlas" });
    } catch (err) {
  ...
  app.post('/api/distribute', async (req, res) => {
    try {
      const { adId } = req.body;
      if (!adId) return res.status(400).json({ error: 'Missing ad ID' });
  
      const ad = await Campaign.findById(adId);
      ...
      for (const site of websites) {
        // Match logic: ad.category matches site's placementType, and ad.budget covers site's price
        const alreadyMatched = await Match.findOne({ adId: ad._id, websiteId: site._id });
        if (
          !alreadyMatched &&
          ad.category.toLowerCase() === site.placementType.toLowerCase() &&
          ad.budget == site.price
        ) {
      ...
      // Optionally update ad status to approved
      ad.status = 'approved';
      await ad.save();
  
      res.json({ message: 'Ad distributed successfully', matches });
    } catch (error) {
      console.error('Ad distribution error:', error);
      res.status(500).json({ error: 'Failed to distribute ad' });
    }
  });

  app.get('/api/ad/:websiteId', async (req, res) => {
    const { websiteId } = req.params;
  
    const match = await Match.findOne({ websiteId }).populate('adId');
    if (!match) return res.status(404).json({ error: 'No matched ad found' });
  
    const ad = match.adId;
    res.json({
      image: ad.generatedImage,
      title: ad.title,
      link: ad.targetUrl || "#"
    });
  });
